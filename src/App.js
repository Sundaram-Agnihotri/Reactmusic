...
function App() {
  ...
  useEffect(() => {
    ...
    try {
      axios
        .get(
          `https://de1.api.radio-browser.info/json/stations/bycountry/india?order=clickcount&reverse=true`
        )
  ...
  return (
    <div className="App min-h-screen bg-gray-900 text-gray-100 py-4">
      <h1 className="text-2xl font-bold text-gray-100 pb-4">
        Trending Indian Stations
      </h1>
            ...
            <div
              ...
              ${
                currentStation?.url === station.url
                  ? "bg-green-500"
                  : "hover:bg-gray-600 hover:border-gray-100`"
              }`}
            >
              <div
                className="flex items-center justify-center h-12 w-12 rounded-full bg-gray-700 mr-4 cursor-pointer"
              ...
              <h2
                className={`text-sm font-semibold title cursor-pointer ${
                  currentStation?.url === station.url
              ...
              {currentStation?.url === station.url &&
                (isPlaying ? (
                  <button onClick={handlePause} className="ml-auto mr-4">
                    <FaPause
                      className="h-6 w-6 hover:text-green-200"
                      style={{ strokeWidth: 1 }}
                    />
                  </button>
                ) : (
